...
class ple_5_2 (osv.Model):
    _name = "l10n_pe.ple_5_2"
    _inherit = "l10n_pe.ple"

    _columns= {
        'lines_ids': fields.one2many ('l10n_pe.ple_line_5_2', 'ple_5_2_id', 'Lines', readonly=True, states={'draft':[('readonly',False)],}),
    }

    def action_reload (self, cr, uid, ids, context=None):
        assert ids and len(ids)==1
        ple_line_obj = self.pool.get('l10n_pe.ple_line_5_2')
        ...
        move_lines = self.get_move_lines (cr, uid, ple.period_id.id, '5_2', ple.company_id.id, context=context)
    ...
    def get_output_filename (self, cr, uid, ids, context=None):
        return "sunat_5_2.txt"

class ple_line_5_2 (osv.Model):
    _name = 'l10n_pe.ple_line_5_2'
    _inherit = 'l10n_pe.ple_line'
    
    _columns = {
        'ple_5_2_id': fields.many2one('l10n_pe.ple_5_2', 'PLE', on_delete='cascade'),
        'state': fields.related ('ple_5_2_id', 'state', string='State', type="char"),
        'company_id': fields.related ('ple_5_2_id', 'company_id', string='Company', type="many2one", relation="res.company"),
...
class ple_configuration (osv.osv):
    ...
    def get_report_type (self, cr, uid, context=None):
        ...
        rep_types.append (('5_2', '5.2 Libro Diario Simplificado'))

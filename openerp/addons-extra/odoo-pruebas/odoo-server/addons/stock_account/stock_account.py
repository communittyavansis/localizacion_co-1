...
class stock_quant(osv.osv):
    ...
    def _prepare_account_move_line(self, cr, uid, move, qty, cost, credit_account_id, debit_account_id, context=None):
        ...
        if context.get('force_valuation_amount'):
            ...
            if move.product_id.cost_method == 'average':
                #Entrada
                valuation_amount = move.location_id.usage != 'internal' and move.location_dest_id.usage == 'internal' and move.price_unit or cost or move.product_id.standard_price
            else:
        ...
        if move.location_id.usage != 'internal' and move.location_dest_id.usage == 'internal':
           #Entradas
           vals = {'cost': move.product_id.standard_price,
                   'unit_price': move.price_unit,
                  }
        else:
           #Salidas
           vals = {'cost': valuation_amount,
                   'unit_price': move.procurement_id.sale_line_id and move.procurement_id.sale_line_id.price_unit or 0.00,
                  }
        
        stock_move_obj.write(cr, uid, [move.id], vals, context=context)
    ...
    def _create_account_move_line(self, cr, uid, quants, move, credit_account_id, debit_account_id, journal_id, context=None):
        ...
        for cost, qty in quant_cost_qty.items():
            ...
            #Adiciona el quant_id
            debit_line_vals = {'quant_id' : quant_id,
                          }
            credit_line_vals = {'quant_id' : quant_id,
                           }
            move_lines[0][2].update(debit_line_vals)
            move_lines[1][2].update(credit_line_vals)
            
            period_id = context.get('force_period', self.pool.get('account.period').find(cr, uid, move.date, context=context)[0])
            mov = move_obj.create(cr, uid, {'journal_id': journal_id,
                                      'line_id': move_lines,
                                      'period_id': period_id,
                                      'date': move.date,
                                      #'date': fields.date.context_today(self, cr, uid, context=context),
        ...
        #Actualiza la relación stock.move - account.move
        stock_move_obj = self.pool.get('stock.move')
        vals = {'account_move_ids': [(4, mov)],
	       }
        stock_move_obj.write(cr, uid, [move.id], vals, context=context)
        ...
        for move in self.browse(cr, uid, ids, context=context):
            ...
            if ( 
                 (move.location_id.usage in ('supplier','customer')) 
                  ...
                  and (move.additional_cost == False) 
                ) or (
                     (move.location_id.usage == 'internal') 
                      and (move.product_id.cost_method == 'average') 
                      and (move.additional_cost == True)
                ):

                product = move.product_id
		if move.additional_cost:
			product_avail = product.qty_available - move.product_qty	
		else:

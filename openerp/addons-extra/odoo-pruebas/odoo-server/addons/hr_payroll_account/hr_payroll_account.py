...
class hr_payslip(osv.osv):
    ...
    def process_sheet(self, cr, uid, ids, context=None):
        move_pool = self.pool.get('account.move')
        emp_contract = self.pool.get('hr.contract.setting')
        ...
        for slip in self.browse(cr, uid, ids, context=context):
            ...
            for line in slip.details_by_salary_rule_category:
                ...
                #aca se captura el el partner de la tabla de contartos
                contrib_id = emp_contract.search(cr, uid, [('contract_id', '=',slip.employee_id.contract_id.id),('contrib_id','=',line.salary_rule_id.register_id.id)], context=context)
                partner_contrib_id = False
                account_deb_contrib_id = False
                account_cre_contrib_id = False
                if len(contrib_id) > 0:
                    partner_contrib_id = emp_contract.browse(cr, uid, contrib_id[0], context=context).partner_id.id
                    account_deb_contrib_id = emp_contract.browse(cr, uid, contrib_id[0], context=context).account_debit_id
                    account_cre_contrib_id = emp_contract.browse(cr, uid, contrib_id[0], context=context).account_credit_id
                partner_id = line.salary_rule_id.register_id.partner_id and line.salary_rule_id.register_id.partner_id.id or partner_contrib_id or  default_partner_id
                debit_account_id = line.salary_rule_id.account_debit or account_deb_contrib_id 
                credit_account_id = line.salary_rule_id.account_credit or account_cre_contrib_id

                if debit_account_id and debit_account_id.id:
					
					tercero = debit_account_id.type in ('receivable', 'payable') and partner_id or default_partner_id
                    ...
                    'partner_id': debit_account_id.type in ('receivable', 'payable') and partner_id or default_partner_id,
                ...
                if credit_account_id and credit_account_id.id:
					
					tercero = credit_account_id.type in ('receivable', 'payable') and partner_id or default_partner_id
                    ...
                    'partner_id': credit_account_id.type in ('receivable', 'payable') and partner_id or default_partner_id,

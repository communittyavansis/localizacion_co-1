...
class hr_payslip_run(osv.osv):
    ...
    def generate_payslip_run(self, cr, uid, ids, context=None):
		...
		for payslip in self.browse(cr,uid,ids,context=context):
                    if payslip.slip_ids:
                       for run in payslip.slip_ids:
                           #print '-----------------------------'
                           #print run.number 
		           #slip_obj.process_sheet(cr, uid, run.id, context=context)
                           slip_obj.compute_sheet(cr, uid, run.id, context=context)
    ...
    def get_payslip_lines(self, cr, uid, contract_ids, payslip_id, context):
        ...
        class Payslips(BrowsableObject):
            ...
            def sum_mount(self, code, from_date, to_date):
                self.cr.execute("SELECT COALESCE(sum(pl.total),0) as suma FROM hr_payslip as hp, hr_payslip_line as pl, hr_salary_rule_category hc WHERE hp.id = pl.slip_id AND pl.category_id = hc.id AND hp.contract_id = %s AND hp.date_from >= (to_char(%s::date,'YYYY-mm')||'-01')::date\
                                  AND hp.date_to <= (to_char(%s::date,'YYYY-mm')||'-15')::date AND hc.code = %s",(self.contract_id.id, from_date, to_date, code))
            ...
            def sum_rule(self, rule_id, from_date, to_date):
                self.cr.execute("SELECT COALESCE(sum(pl.total),0) as suma FROM hr_payslip as hp, hr_payslip_line as pl, hr_salary_rule_category hc WHERE hp.id = pl.slip_id AND pl.category_id = hc.id AND hp.contract_id = %s AND hp.date_from >= (to_char(%s::date,'YYYY-mm')||'-01')::date\
                                  AND hp.date_to <= (to_char(%s::date,'YYYY-mm')||'-15')::date AND pl.salary_rule_id = %s",(self.contract_id.id, from_date, to_date, rule_id))

                res = self.cr.fetchone()
                return res and res[0] or 0.0

            def sum_days(self, from_date, to_date):
                self.cr.execute("SELECT coalesce(number_of_days,0) as dias from hr_payslip_worked_days hd, hr_payslip as hp where hd.code = 'WORK100' AND hp.id = hd.payslip_id AND hp.contract_id = %s AND hp.date_from >= (to_char(%s::date,'YYYY-mm')||'-01')::date\
                                  AND hp.date_to <= (to_char(%s::date,'YYYY-mm')||'-15')::date",(self.contract_id.id, from_date, to_date))

...
class account_invoice_tax(osv.osv):
    ...
    def compute(self, invoice):
        ...
        for k in tax_grouped.keys():
            apply = True
            if k[0]:
                apply = obj_tax._applicable_invoice(cr, uid, k[0], invoice.id, tax_grouped[k]['amount'], tax_grouped[k]['base'], context=context)
            if k[1]:
                apply = apply and obj_tax._applicable_invoice(cr, uid, k[1], invoice.id, tax_grouped[k]['amount'], tax_grouped[k]['base'],context=context)
            if not apply:
                tax_grouped.pop(k)

...
class hr_expense_expense(osv.osv):
    ...
    def action_move_create(self, cr, uid, ids, context=None):
        ...
        for exp in self.browse(cr, uid, ids, context=context):
            ...
            if exp.type.type in  ('rembolso_gastos', 'rembolso_caja_menor') :
                acc = exp.employee_id.address_home_id.property_account_payable.id
                valor_total = total

            elif exp.type.type == 'tarjeta_credito' :
                acc = exp.journal_id.default_credit_account_id.id
                valor_total = total

            elif  exp.type.type == 'anticipo' :

                acc = exp.journal_id.default_credit_account_id.id
                valor = exp.advance_id.amount or 0.0
                valor_total = valor*-1
                if valor > abs(total) :
                   eml.append({
                    'type': 'dest',
                    'name': 'Cuenta por cobrar empleado',
                    'price': valor- abs(total), 
                ...
                elif valor < abs(total) :
                   eml.append({
                    'type': 'dest',
                    'name': 'Cuenta por pagar empleado',
                    'price': ( abs(total)-valor ) * -1, 
            ...
            eml.append({
                    'type': 'dest',
                    'name': 'Legalizacion ant o reembolso',
                    'price': valor_total, 
                    'account_id': acc, 
                    'partner_id' : exp.employee_id.address_home_id.id,
                    'date_maturity': exp.date_confirm, 

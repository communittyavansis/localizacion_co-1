...
class sale_order_line(orm.Model):
    ...
    def _sumar_stock_real(self, cr, uid, ids, product_id, warehouse_id, context):
		
        if product_id and warehouse_id:
			cr.execute("SELECT s.qty FROM stock_quant s INNER JOIN stock_warehouse h on (h.lot_stock_id = s.location_id) WHERE  h.id = %s AND s.reservation_id is null AND s.product_id = %s",(warehouse_id, product_id))
			res = cr.fetchone()
			return res[0] or 0.0
        return 0.0   
    ...
    _columns = {
        ...
        'procurement_group_id': fields.many2one(
            ...
            'Procurement group',
            copy=False),
        'available_real' : fields.function(_sumar_stock_real, type='float', string='Stock real', readonly=True),
    }
    
    
    def onchange_product_id(self, cr, uid, ids, product_id, warehouse_id, context):
        valor = self._sumar_stock_real(cr, uid, ids, product_id, warehouse_id,context=context)
        return {'value': {'available_real': valor}}
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        if product  and context.get('default_warehouse_id', False) :
            wh_id = context['default_warehouse_id']
            if wh_id:
               ctx = context.copy()
               ctx['default_warehouse_id'] = wh_id
               valor = self._sumar_stock_real(cr, uid, ids, product, wh_id,context=ctx)
               res['value'].update({'available_real': valor})
        return res
